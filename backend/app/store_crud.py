...
from store_models import (
    Store, StoreBase,
    Product, ProductBase,
    ProductAvailability, ProductAvailabilityBase,
    Order, OrderBase,
    OrderItem, OrderItemBase,
    FoodItem, FoodItemBase,
...
def update_store(*, session: Session, db_store: Store, store_update: StoreBase) -> Store:
...
def update_product(*, session: Session, db_product: Product, product_update: ProductBase) -> Product:
...
def create_product_availability(*, session: Session, pa_create: ProductAvailabilityBase) -> ProductAvailability:
...
def get_product_availability_by_id(*, session: Session, pa_id: int) -> ProductAvailability | None:
    """Retrieves a product availability entry by its ID."""
    statement = select(ProductAvailability).where(ProductAvailability.id == pa_id)
    return session.exec(statement).first()

def get_product_availability_for_store_product(*, session: Session, store_id: int, product_id: int) -> ProductAvailability | None:
...
def update_product_availability(*, session: Session, db_pa: ProductAvailability, pa_update: ProductAvailabilityBase) -> ProductAvailability:
...
def update_order(*, session: Session, db_order: Order, order_update: OrderBase) -> Order:
...
def create_order_item(*, session: Session, order_item_create: OrderItemBase) -> OrderItem:
...
def update_order_item(*, session: Session, db_order_item: OrderItem, order_item_update: OrderItemBase) -> OrderItem:
...
def update_food_item(*, session: Session, db_food_item: FoodItem, food_item_update: FoodItemBase) -> FoodItem:
...
def create_price_history(*, session: Session, price_history_create: PriceHistoryBase) -> PriceHistory:
...
def get_price_history_by_id(*, session: Session, price_history_id: int) -> PriceHistory | None:
    """Retrieves a price history entry by its ID."""
    statement = select(PriceHistory).where(PriceHistory.id == price_history_id)
    return session.exec(statement).first()

def get_price_history_for_food_item_and_store(*, session: Session, food_item_id: int, store_id: int) -> list[PriceHistory]:
    """Retrieves price history for a specific food item and store."""
    statement = select(PriceHistory).where(
        PriceHistory.food_item_id == food_item_id,
...
def update_price_history(*, session: Session, db_price_history: PriceHistory, price_history_update: PriceHistoryBase) -> PriceHistory:
...
def delete_price_history(*, session: Session, price_history_id: int) -> bool:
    ...
    price_history = get_price_history_by_id(session=session, price_history_id=price_history_id)
    if price_history:
